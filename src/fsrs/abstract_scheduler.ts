...
export abstract class AbstractScheduler implements IScheduler {
  ...
  private init() {
    ...
    if (state !== State.New && last_review) {
      interval = this.review_time.diff(last_review as Date, 'days') // last review interval
    }
    this.current.last_review = this.review_time // last review of current card is now
  ...
  protected buildLog(rating: Grade): ReviewLog {
    ...
    // build log of current, not future cards 
    return {
      rating: rating,
