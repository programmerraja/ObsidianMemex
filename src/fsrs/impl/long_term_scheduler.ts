...
export default class LongTermScheduler extends AbstractScheduler {
  protected override newState(grade: Grade): RecordLogItem {
    ...
    // Updates scheduled days and due date
    this.next_interval(
      next_again,
      next_hard,
      next_good,
  ...
  // Initializing difficulty and stability
  private init_ds(
    next_again: Card,
    next_hard: Card,
    next_good: Card,
  ...
  /**
   * Review/New next_interval using Stability, 
   * interval is only used in fuzz
   */
  private next_interval(
    ...
    next_good: Card,
    next_easy: Card,
    interval: number // at the beginning interval is just 0, this is used in fuzz
  ): void {
  ...
  private update_next(
    ...
    // setting the next cards based on result
